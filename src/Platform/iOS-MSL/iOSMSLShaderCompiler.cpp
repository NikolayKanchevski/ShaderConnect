...
#include "iOSMSLShaderCompiler.h"
...
namespace ShaderConnect
{
    ...
    void iOSMSLShaderCompiler::CompileShader(const std::vector<uint32> &spirvBuffer, const std::filesystem::path &outputShaderFilePath)
    {
        ...
        File::WriteToFile(outputShaderFilePath.string() + ".ios.msl", mslCode.data(), mslCode.size() * sizeof(char), true, true);
