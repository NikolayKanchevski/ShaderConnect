...
namespace ShaderConnect
{
    ...
    void ESSLShaderCompiler::CompileShader(const std::vector<uint32> &spirvBuffer, const std::filesystem::path &outputShaderFilePath)
    {
        ...
        File::WriteToFile(outputShaderFilePath.string() + ".essl", glslCode.data(), glslCode.size() * sizeof(char), true, true);
    }
